...
float rate_K_mbest_encode(int *indexes, float *x, float *xq, int ndim,
                          int mbest_entries) {
  int i, j, n1, n2;
  const int16_t *codebook1 = newamp1vq_cb[0].cb;
  const int16_t *codebook2 = newamp1vq_cb[1].cb;
  ...
  for (j = 0; j < mbest_entries; j++) {
    ...
    for (i = 0; i < ndim; i++) target[i] = x[i] - codebook1[ndim * n1 + i] / newamp1vq_cb[0].scale;
  ...
  mse = 0.0;
  for (i = 0; i < ndim; i++) {
    tmp = codebook1[ndim * n1 + i] / newamp1vq_cb[0].scale +
          codebook2[ndim * n2 + i] / newamp1vq_cb[1].scale;
    mse += (x[i] - tmp) * (x[i] - tmp);
    xq[i] = tmp;
  }
...
void newamp1_indexes_to_rate_K_vec(float rate_K_vec_[],
  ...
  if (user_rate_K_vec_no_mean_ == NULL) {
    /* normal operation */
    for (k = 0; k < K; k++) {
      rate_K_vec_no_mean_[k] = codebook1[K * n1 + k] / newamp1vq_cb[0].scale +
                                codebook2[K * n2 + k] / newamp1vq_cb[1].scale;
    }
  } else {
  ...
  *mean_ = newamp1_energy_cb[0].cb[indexes[2]] / newamp1_energy_cb[0].scale;
