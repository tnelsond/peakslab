...
long quantise(const int16_t *cb, float vec[], float w[], int k, int m, float *se)
...
int find_nearest(const int16_t *codebook, int nb_entries, float *x, int ndim) {
  int i, j;
  float min_dist = 1e15;
  int nearest = 0;
...
int find_nearest_weighted(const int16_t *codebook, int nb_entries, float *x,
...
void lspjmv_quantise(float *x, float *xq, int order) {
  ...
  const int16_t *codebook1 = lsp_cbjmv[0].cb;
  const int16_t *codebook2 = lsp_cbjmv[1].cb;
  const int16_t *codebook3 = lsp_cbjmv[2].cb;
...
void quantise_WoE(C2CONST *c2const, MODEL *model, float *e, float xq[]) {
  ...
  float err[2];
  float w[2];
  const int16_t *codebook1 = ge_cb[0].cb;
