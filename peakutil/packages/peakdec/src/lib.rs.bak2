...
impl PeakDecoder {
    ...
    pub fn new(input: &[u8], is_compressed: bool) -> Result<PeakDecoder, JsValue> {
        ...
        Ok(PeakDecoder {
            ...
            results: vec![Vec::new(); ni + 2],
            populated: vec![false; ni + 2],
            ri: vec![0; ni + 2],
            reverse: false,
        })
    }
    ...
    pub fn get_results_from_level(&mut self, level: usize, count: usize) -> Vec<Uint8Array> {
        ...
        if !self.populated[level] {
            self.populate_level(level);
        }

        let mut out = Vec::with_capacity(count);
        let start = self.ri[level];
        let end = (start + count).min(self.results[level].len());

        for &locali in &self.results[level][start..end] {
    ...
    fn populate_level(&mut self, level: usize) {
        let ni = self.offsets_starts.len() - 2;
        let query_len = self.query_lower_bytes.len();

        // For very short queries, skip expensive fallbacks to prevent explosion
        if level >= ni && query_len < 3 {
            self.results[level] = Vec::new();
            self.populated[level] = true;
            return;
        }
        ...
        if level < ni {
            ...
            let primary_len = self.lens[0];
            for locali in 0..primary_len {
                let off = self.get_offset_at(0, locali);
                let key_bytes = unsafe { get_key_bytes(&self.binary, self.data_start, off) };
                let already_in = (0..ni).any(|i| self.populated[i] && self.results[i].contains(&(locali as u32)));
                if !already_in && key_lower_matches(key_bytes, query_lower_bytes, false) {
                    temp_results.push(locali as u32);
                }
            }
        } else {
            // Full-text fallback
            ...
            for locali in 0..primary_len {
                let start_off = self.get_offset_at(0, locali);
                let end_off = if locali + 1 < primary_len {
                    self.get_offset_at(0, locali + 1)
                } else {
                    (self.binary.len() - self.data_start) as u32
                };
        ...
        self.results[level] = temp_results;
        self.populated[level] = true;
    }
}
